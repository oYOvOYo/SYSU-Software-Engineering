...
void iteration_unit(uint64_t* block, uint64_t sub_key) {
  uint64_t left = (*block) & mask_uint64_from_to(0, 31);
  uint64_t right = ((*block) & mask_uint64_from_to(32, 63)) << 32;

  uint64_t new_left = right;
  map_uint64(&right, MESSAGE_EXPANSION, 48);
  uint64_t new_right = left ^ (right ^ sub_key);
}
