    ...
    def main_fuc(self, file_adress):
        ...
        for x in self.word_list:
            if not x in self.answer:
                ...
                self.answer[x] += 1
        self.answer = sorted(self.answer.iteritems(), key=lambda x: x[
                             0].upper())  # 无关大小写的字典序排序
        self.answer = sorted(self.answer, key=lambda x: x[
                             1], reverse=True)  # 出现次数排序
        self.answer = filter(
            lambda x: not x[0] in special_char, self.answer)  # 删除单个存在的特殊字符
