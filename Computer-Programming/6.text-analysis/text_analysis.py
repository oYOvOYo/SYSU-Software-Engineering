...
class text_analysis(object):
    ...
    def main_fuc(self, file_adress):
        ...
        delete_char = (string.punctuation +
                       string.digits).translate(None, special_char)  # 需要删除的字符
        ...
        for each_file_adress in file_adress:
            ...
            self.data = "".join(
                map(lambda x: x in delete_char and ' ' or x, self.data))   # 删除字符
            self.word_list = self.data.split()  # 切片
            self.word_list = map(lambda x: string.strip(
                x, special_char), self.word_list)  # 删除开头结尾的独立存在的特殊字符
            ...
            if self.word_set == None:  # 对于第一次处理的文章
                self.word_set = set(
                    map(lambda x: x[0], self.new_dict.iteritems()))  # 储存所有单词
                self.ans = self.new_dict  # 旧dict就是本身
            else:  # 对于第二次及以后的文章，每次求交集
                self.word_set = self.word_set & set(
                    map(lambda x: x[0], self.new_dict.iteritems()))
        ...
        # 全部处理以后只需要对总结果进行一次排序
        self.ans = sorted(self.ans.iteritems(), key=lambda x: x[
                          0].upper())  # 无关大小写的字典序排序
        self.ans = sorted(self.ans, key=lambda x: x[1], reverse=True)  # 出现次数排序
        self.ans = filter(
            lambda x: not x[0] in special_char, self.ans)  # 删除单个存在的特殊字符
        # 按照格式输出
